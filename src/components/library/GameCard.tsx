...
import { cn, formatDate, pluralizeClips } from "@/lib/utils";
...
export function GameCard({ game, index }: { game: Game; index: number }) {
  ...
  return (
    ...
    >
      ...
      >
        <div className={cn(
          "relative aspect-[3/4.25] overflow-hidden rounded-[1.2rem] border border-white/10 bg-card shadow-[0_24px_70px_rgba(0,0,0,.25)]",
          performanceMode
            ? "transition-transform duration-150 group-hover:-translate-y-0.5"
            : "transition duration-500 group-hover:-translate-y-1.5 group-hover:border-white/20 group-hover:shadow-[0_30px_80px_rgba(0,0,0,.45)]",
        )}>
          <GameArtwork
            title={game.title}
            ...
            imageUrl={posterUrl}
            className={cn("size-full", performanceMode ? "transition-transform duration-150 group-hover:scale-[1.01]" : "transition duration-700 group-hover:scale-[1.025]")}
          />
